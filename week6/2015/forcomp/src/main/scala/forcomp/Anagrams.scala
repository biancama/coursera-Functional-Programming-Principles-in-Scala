...
object Anagrams {
  ...
  /** The `dictionaryByOccurrences` is a `Map` from different occurrences to a sequence of all
    ...
    *
    */
  lazy val dictionaryByOccurrences: Map[Occurrences, Sentence] = dictionary groupBy (wordOccurrences(_))
  ...
  def subtract1(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.toMap.map(s => {
      val amount = s._2 - yMap.getOrElse(s._1, 0)
      //if (amount > 0)
      (s._1, amount)
    }
    ).toList .filter { case (c, i) => i > 0 }
  }
  ...
  def sentenceAnagramsMemo(sentence: Sentence): List[Sentence] = {
    ...
    def iter(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        cache.getOrElseUpdate(occurrences, for {
              combination <- combinations( occurrences )
              word <- dictionaryByOccurrences getOrElse (combination, Nil)
              sentence <- iter( subtract(occurrences, wordOccurrences(word)) )
              if !combination.isEmpty
